...
def test_the_report_counts_the_stumbles_it_cut() -> None:
    assert "Stumbles cut       1" in report_with_a_stumble()
...
def test_a_cut_with_no_stumble_in_it_tabulates_nothing() -> None:
    report = report_for(CLEAN_READ)

    assert "Stumbles cut       0" in report
